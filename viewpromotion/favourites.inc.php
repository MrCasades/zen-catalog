...
if (isset($_POST['val_fav']) && isset($_POST['id']) && isset($_POST['idauthor']) && $_POST['val_fav'] == 'addfav')
{
	...
	$favData = 'SELECT id, promotion, promotiontitle, promotiondate, imghead, imgalt, idauthor, idcategory FROM promotion WHERE id = '.$_POST['id'];//подготовка данных для избранного
	...
	try
	{
		...
		$post = implode(' ', array_slice(explode(' ', strip_tags($row['promotion'])), 0, 50));
		$postTitle = $row['promotiontitle'];
		$postDate = $row['promotiondate'];
		$imgHead = $row['imghead'];
		$imgAlt = $row['imgalt'];
		$idAuthorPost = $row['idauthor'];
		$idCategory = $row['idcategory'];
		$url = '<a href="//'.MAIN_URL.'/viewpromotion/?id='.$row['id'].'" class="btn btn-primary">Далее</a>';
	...
	$updateCount = 'UPDATE promotion SET favouritescount = favouritescount + 1 WHERE id = ';
