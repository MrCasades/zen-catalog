...
buttonAnimation ('btn_recomm', 'rec_2.jpg', 'rec.jpg')
buttonAnimation ('butt_act', 'act_2.jpg', 'act.jpg')
buttonAnimation ('butt_link', 'lin_2.jpg', 'lin.jpg')

//Функция подсветки кнопки
function buttonAnimation (buttId, mousoverPic, mouseoutPic)
{
	const cursorHier = document.getElementById(buttId)

	cursorHier.addEventListener('mouseover', (event) => {cursorHier.src = mousoverPic

	}) 

	cursorHier.addEventListener('mouseout', (event) => {cursorHier.src = mouseoutPic
		...
		function(event){
			...
			if (confRecomm === false){
				...
				if (parseInt($("#score").html()) < parseInt($("#recommprice").attr('value'))){
					...
					$("#ajax_form_recomm").html('');
					alert('Вы успешно рекомендовали канал! Теперь он на главной странице на 1-м месте!');
				}
